...
class Mongo extends Connection
{
    ...
    public function connect(array $config = [], $linkNum = 0)
    {
        if (!isset($this->links[$linkNum])) {
            ...
            if ($config['is_set']){
                $config['dsn'] .='/'.$config['database'].'?replicaSet='.$config['replicaSet'];
            }
     ...
     * @throws RuntimeException
     */
    public function command($command, string $dbName = '', ReadPreference $readPreference = null, $typeMap = null, bool $master = false): array
    {
        if (is_array($command)){
            $command['cursor']=new \ArrayObject();
